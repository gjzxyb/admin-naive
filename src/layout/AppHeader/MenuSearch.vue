<template>
  ...
  >
    <template #default="{ item }">
      <router-link :to="item.menu_path" custom v-slot="{ href }">
        <el-link class="menu-link" :href="href" :disabled="item.type === 1" :underline="false">
          <span class="link-text" @click="handleLinkClick(item, $event)">{{ item.menu_name }}</span>
        </el-link>
      </router-link>
    </template>
  </el-autocomplete>
